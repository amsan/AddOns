...
function GenericTrigger.CreateFallbackState(data, triggernum, state)
  ...
  if (event.textureFunc ) then
    local ok, texture, atlas = xpcall(event.textureFunc, geterrorhandler(), data.trigger);
    state.texture = ok and texture or nil;
    state.atlas = ok and atlas or nil;
  end
  
  if (event.stacksFunc) then
    local ok, stacks = event.stacksFunc(data.trigger);  -- Derangement wonders if it's a bug to not use xpcall here
