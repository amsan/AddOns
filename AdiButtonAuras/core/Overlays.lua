...
local conditionalsCache = addon.Memoize(function(index)
	...
	--support for long macros (added by Derangement)
	local longMacro = strmatch(body,"^#longmacro.*(LongMacro_[^;%s]*)");
	if( longMacro ) then 
		longMacro = _G[longMacro];
		body = (longMacro and longMacro:GetAttribute("macrotext")) or body;
	end
...
function overlayPrototype:UpdateDynamicUnits(event, unit)
	...
	for token, conditional in pairs(self.unitConditionals) do
		...
		if not unit or unit == "" then
			...
			local default = addon.dynamicUnitConditionals[token];	--start of chunk modified by Derangement
			if( default ) then
				_, unit = SecureCmdOptionParse(default);
			end
			
			if not unit or unit == "" then
				if token == "enemy" then							--start of original code
				...
				elseif token == "ally" then
					...
					unit = ""
				end													--end of original code
			end														--end of chunk modified by Derangement
